...
const CategorySidebar = ({ selectedCategory, handleSelectCategory, menuItems, categories, sortBy, setSortBy }) => (
  <Paper elevation={2} sx={{ p: 3, height: '100%', position: 'sticky', top: '90px' }}>
    <Typography variant="h6" sx={{ mb: 3, fontWeight: 'bold', color: '#4a148c' }}>Menu Categories</Typography>
    ...
    <List component="nav" sx={{ mb: 4 }}>
      ...
      {categories?.map((category) => {
        ...
        return (
          <Zoom in={true} key={category._id} style={{ transitionDelay: `${50 * categories.indexOf(category)}ms` }}>
            <ListItemButton 
